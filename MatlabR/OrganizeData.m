...
writetable(T_HS1, 'T_EquilumHS.csv')
...
writetable(T_Exp1Data, 'T_EquilumData.csv')




%% ORGANIZE REPLICATION AND MATCH DATA
...
writetable(T_HS2, 'T_Equilum2HS.csv')
...
%coding -1 and 1 for the chroma factor for the matched dataset
Chroma2 = [ones(8,1)*(-1); ones(8,1); ones(8,1)*(-1); ones(8,1)];
T_Exp2Data.C_m = repmat(Chroma2, n2,1);

% coding the matched and unmatched (c*=20) to have the a* and b* for c* = 20 to
% test whether the fit of the a* and b* coordiantes when chroma = 20 is
% weaker for the matched colors 1-8 is light/c*=20 17:34 is dark c* = 20.
c20coords = [1:8, 1:8, 17:24, 17:24];
T_Exp2Data.b_m = repmat(outM.b(c20coords), n2,1);
T_Exp2Data.a_m = repmat(outM.a(c20coords), n2,1);

%Write out the table
writetable(T_Exp2Data, 'T_Equilum2Data.csv')
...
writetable(T_avgs, 'T_Equilum1+2avgs.csv')
